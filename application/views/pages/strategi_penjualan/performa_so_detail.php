...
<div class="container-xxl flex-grow-1 container-p-y">
    <h5 class="fw-bold text-warning py-3 mb-4"><span class="text-muted fw-light">Strategi Penjualan /</span>
        <span class="text-muted fw-light">Performa SO /</span> Performa SO Detail
    </h5>
    <div class="row">
        <div class="col-md-12">
            <div class="card mb-4">
                ...
                <h5 class="card-header text-dark fs-4 text-start">
                    ...
                    <p style="font-size: 38px;margin-top:10px;">
                        <b><?= $performa_detail_month[0]->nama_so;?></b>
                    </p>
...
<script defer>
    ...
    $(document).ready(function() {
        ...
        function show_content(filter = '', subFilter = '') {
            $.ajax({
                type: "POST",
                url: '<?php echo base_url(); ?>/strategi_penjualan/performa_so_detail/chartdata',
                data: {
                    'id_so': <?= $performa_detail_month[0]->id_so;?> ,
                    'params': 'curr_month',
                    'filter': filter,
                    'subFilter': subFilter
                },
            ...
            $.ajax({
                ...
                success: function(res) {
                    chart_performa_so_detail_ytd.updateSeries([{
                        ...
                        type: 'column',
                        data: (res.data_values2.slice(0, res.data_fields1.length)).map(
                            bFormatter)
                    }, {
        ...
        $('#filter-btn').click(function() {
            ...
            if (filter_values != '' && subFilter_values != '') {
                chart_performa_so_detail_mtd.resetSeries()
                chart_performa_so_detail_ytd.resetSeries()
                $('#performa_so_detail_mtd_table').DataTable().destroy();
                $('#performa_so_detail_ytd_table').DataTable().destroy();
                show_content(filter_values, subFilter_values);
            } else {
