                    ...
                    <div class="ms-4 me-4 mb-4">
                        <table id="performa_produk_mtd_table" class="table table-striped table-hover display nowrap"
                            ...
                            <tbody>
                                <?php 
                                    $no=1; 
                                    foreach ($current_month_product as $key=>$row) { ?>
                                    <tr>
                                        <td>
                                            <?= $no++;?>
                                        </td>
                                        ...
                                        <td class="get_val">
                                            <?= htmlentities($last_month_product[$key]->mtd_amt);?>
                                        </td>
                                        <td class="get_val">
                                            <?= htmlentities($row->mtd_amt);?>
                                        </td>
                                        <td >
                                        <?= htmlentities($last_month_product[$key]->mtd_unit);?>
                                        </td>
                                        <td >
                                            <?= htmlentities($row->mtd_unit);?>
                                        </td>
                                        <td><button onclick="window.location.href='<?= site_url('performa_produk_detail/'.$CI->security_idx->encrypt_url($row->id_gp))?>';sessionStorage.setItem('is_mtd', true);"
                                                type="button"
                        ...
                        <table id="performa_produk_ytd_table" class="table table-striped table-hover display nowrap"
                            ...
                            <?php 
                                    ...
                                    <tr>
                                        ...
                                        <td><button onclick="window.location.href='<?= site_url('performa_produk_detail/'.$CI->security_idx->encrypt_url($row->id_gp))?>';sessionStorage.setItem('is_mtd', false);"
                                                type="button"
                                                class="btn_session badge btn btn-primary me-2"><i
                                                    class='bx bx-detail me-1'></i>
                                                Detail</button></td>
...
<script async>
    <?php
        ...
        foreach($current_month_product as $key=>$row) {
            $items_mtd[] = htmlentities($row -> gp);
            $lending_mtd[] = htmlentities($row -> mtd_amt);
            $lending_prev_mtd[] = htmlentities($last_month_product[$key]->mtd_amt);
        }
        foreach($current_year_product as $key=>$row) {
            $items_ytd[] = htmlentities($row -> gp);
            $lending_ytd[] = htmlentities($row -> mtd_amt);
            $lending_prev_ytd[] = htmlentities($last_year_product[$key]->mtd_amt);
        }
    ?>
    //mtd
