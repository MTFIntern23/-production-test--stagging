...
class Npl_monitoring_model extends CI_Model{
    ...
    public function npl_monitoring($params,$filter='',$subfilter=''){
        if (!$this->session->has_userdata(self::SESSION_KEY)) {
            return null;
        }
        $cabangUser = $this->auth_model->current_user()->id_cabang;
        $this->db->cache_on();
        $this->db->select('YEAR(periode) as year, MONTH(periode) as month,'.$this->_table_npl.'.persentasi as ytd_persentasi');
