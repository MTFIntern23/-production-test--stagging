...
class Product_detail_model extends CI_Model{
    ...
    var $column_order = array(null, 'no_aplikasi','nama_cust','produk','lending_amt','tgl_appin','tgl_golive','jenis','nama_so'); 
    var $column_search = array('no_aplikasi','nama_cust','produk','lending_amt','tgl_appin','tgl_golive','jenis','nama_so'); 
    var $order = array('tgl_golive' => 'desc'); 
    private function _product_detail($id,$params){
    ...
    {
        $this->db->cache_on();
        $this->_product_detail($id,$params);
        if($_POST['length'] != -1)
