...
class Profit_model extends CI_Model{
    ...
    var $column_order = array(null, 'komponen_profit','profit',null); 
    var $column_search = array('komponen_profit','SUM(profit)'); 
    var $order = array('profit' => 'desc'); 
    private function _profit($params){
        if (!$this->session->has_userdata(self::SESSION_KEY)) {
            return null;
        }
        $cabangUser = $this->auth_model->current_user()->id_cabang;
        if($params == 'curr_year_val'||$params == 'last_year_val'){
            $this->db->select('YEAR(periode) AS year,MONTH(periode)as month,'.$this->_table_kp.'.`id_komponen`,komponen_profit,SUM(profit) AS profit');
        }else{
            $this->db->select(''.$this->_table_kp.'.`id_komponen`,komponen_profit,SUM(profit) AS profit');
        }
